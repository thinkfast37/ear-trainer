...
- [X] T107 [US9.3] Mastery dialog and stopping-point visibility (P-049): `src/ui/celebration.js` — the celebration renders as a modal dialog (overlay, `role="dialog"`, visible without scrolling) with two actions: return to the main menu (ends the session, goes home) and keep practising (closes the dialog, session continues); `src/ui/session.js` — the dialog renders when a level masters mid-session; the End button shows the dialog first when a mastery in this session was never shown (AC-9.3.4); `src/styles.css` — modal overlay styles, and the stopping-point toast clears the bottom safe area (`env(safe-area-inset-bottom)`). Implements AC-9.3.2/1–3, AC-9.3.4, AC-2.6.2 (wording), AC-9.2.2/1–2.
