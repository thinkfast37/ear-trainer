...
/** Every item (entry × rotation) at or below a catalog tier (AC-8.2.2), for a texture. */
export function catalogItems(catalog, catalogTier, texture) {
  const out = [];
  for (const e of catalog) {
    if (!e.active || e.level > catalogTier) continue;
...
export function createProgressionsTrack(def, catalog) {
  ...
  return {
    ...
    itemsFor(levelNo) {
      ...
      return l.presentations.flatMap((pres) => catalogItems(catalog, l.catalogTier, pres).map((c) => c.id));
    },
    numeralOptions(levelNo, mode) {
      const level = levelByNo(levelNo);
      const set = new Set();
      for (const c of catalogItems(catalog, level.catalogTier, 'block')) if (c.entry.mode === mode) for (const n of c.numerals) set.add(n);
      if (level.mode === mode) for (const n of level.vocabulary) set.add(n);
      return [...set];
    },
    generate({ levelNo, progress, rng, settings, bias = null, avoid = null, prevKey = null, bassFirst = false, recency = null }) {
      const level = levelByNo(levelNo);
      const items = level.presentations.flatMap((pres) => catalogItems(catalog, level.catalogTier, pres));
      const id = selectItem(rng, progress, items.map((c) => c.id), { bias, avoid, recency });
      const item = items.find((c) => c.id === id);
      const presentation = parseItemId(id).texture;
      ...
      const voiced = presentation === 'block' ? blockVoicings(chords) : voiceLedVoicings(chords, 48 + (presentation === 'arp' ? 0 : 4), 84);
      const exercise = progressionExercise(voiced, presentation === 'arp' ? 'arp' : 'block', { tonicMidi: tonic, mode }, { tempo: settings.arpeggioTempo });
      const answer = item.numerals.slice();
      const options = this.numeralOptions(levelNo, mode);
      const q = { trackId: def.id, levelNo, itemId: id, answer, options, exercise, kind: 'numerals', meta: { key, tonic, mode, presentation, entry: item.entry, rotation: item.rotation, name: item.entry.name, replayLimit: level.replayLimit } };
    ...
    exerciseFor(question, seq, settings = { arpeggioTempo: 120 }) {
      ...
      return progressionExercise(blockVoicings(chords), question.meta.presentation === 'arp' ? 'arp' : 'block', { tonicMidi: question.meta.tonic, mode: question.meta.mode }, { withCadence: false, tempo: settings.arpeggioTempo });
    },
  };
}
