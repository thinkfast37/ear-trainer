 ...
 * the test suites, and reports any AC with no test referencing it. Exits non-zero
 * on a gap, so "every AC has a test" is enforced rather than claimed.
 *
 ...
 *
 * Paths come from spec-trace.config.json at the repository root, so this tool and
 * the spec-trace skill always agree on where the spec and the tests live.
 *
 * Usage:
...
const CONFIG_PATH = join(ROOT, 'spec-trace.config.json');
if (!existsSync(CONFIG_PATH)) {
  console.error(`ac-coverage: spec-trace.config.json not found at the repository root`);
  process.exit(2);
}
const config = JSON.parse(readFileSync(CONFIG_PATH, 'utf8'));
const SPEC = join(ROOT, config.spec);
const TEST_DIRS = (config.testDirs ?? ['tests/unit', 'tests/e2e']).map((d) => join(ROOT, d));
...
if (process.argv.includes('--json')) {
  ...
  if (orphans.length) {
    ...
    console.log('  A renumbered or retired AC leaves its tests behind.');
  }
